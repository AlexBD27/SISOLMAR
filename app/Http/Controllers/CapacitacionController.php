...
class CapacitacionController extends Controller
{
    ...
    //         $xmlFile = $tempPath . '/moodle_backup.xml';
    ...
    //         $xml = simplexml_load_file($xmlFile);
    //         $courseName = (string) ($xml->information->original_course_fullname ?? "Sin nombre");
    //         $backupDate = (string) ($xml->information->backup_date ?? null);
    //         $moodleVersion = (string) ($xml->information->moodle_version ?? null);

    //         $sections = [];
    //         $sectionsPath = $tempPath . '/section';
    //         if (is_dir($sectionsPath)) {
    //             foreach (glob($sectionsPath . '/section_*/section.xml') as $sectionFile) {
    //                 $sectionXml = simplexml_load_file($sectionFile);
    //                 $title = (string) ($sectionXml->title ?? "Sección sin título");

    //                 $mods = [];
    //                 if (isset($sectionXml->mods->mod)) {
    //                     foreach ($sectionXml->mods->mod as $mod) {
    //                         $mods[] = [
    //                             'id' => (string) $mod['id'],
    //                             'type' => (string) $mod['modtype']
    //                         ];
    //                     }
    //                 }
    ...
    //         $activitiesPath = $tempPath . '/activities';
    //         $activityStats = [
    //             'quiz' => 0,
    //             'assign' => 0,
    //             'forum' => 0,
    ...
    //             foreach (glob($activitiesPath . '/*/module.xml') as $moduleFile) {
    //                 $moduleXml = simplexml_load_file($moduleFile);
    //                 $modType = (string) $moduleXml->modulename;

    //                 if (isset($activityStats[$modType])) {
    //                     $activityStats[$modType]++;
    //                 } else {
    //                     $activityStats['other']++;
    //                 }

    //                 // contar preguntas de los quizzes (solo si están embebidas en module.xml)
    //                 if ($modType === 'quiz' && isset($moduleXml->question_instances->question_instance)) {
    //                     $totalQuestions += count($moduleXml->question_instances->question_instance);
    //                 }
    //             }
    //         }

    //         $questionsXml = $tempPath . '/questions.xml';

    //         if (file_exists($questionsXml)) {
    //             $questionsDoc = simplexml_load_file($questionsXml);
